...
import { C, KANBAN_COLS, PRIORITY_COLORS, TAG_COLORS } from "../constants.js";
...
const COL_DOT = {
  "Nova Ideia":        C.textMuted,
  "Em Análise":        C.warning,
  "Aprovado":          C.success,
  "Em Desenvolvimento": C.primary,
};
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 500, color: C.text, lineHeight: 1.45 }}>
          {item.title}
        </span>
        <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
          <div
            style={{
              ...
              boxShadow: `0 0 6px ${PRIORITY_COLORS[item.priority]}88`,
            }}
...
export default function Backlog() {
  ...
  const [form, setForm] = useState({
    title: "", status: "Nova Ideia", tags: [], assignee: "", priority: "média", version: "",
  });
  ...
  async function moveItem(id, newStatus) {
    const r = await authFetch(`/api/backlog/${id}`, {
      method: "PUT",
  ...
  async function handleCreate(e) {
    ...
    setSaving(true);
    setFormErr("");
    const r = await authFetch("/api/backlog", {
      method: "POST",
      body: JSON.stringify({
    ...
    if (r.ok) {
      ...
      setShowForm(false);
      setForm({ title: "", status: "Nova Ideia", tags: [], assignee: "", priority: "média", version: "" });
    } else {
      setFormErr(data.error || "Erro ao criar tarefa");
    }
  }

  return (
    <div style={{ padding: "20px 24px", overflowX: "auto", height: "100%" }}>
      {/* Top bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: C.textSec }}>
            {loading ? "Carregando…" : `${items.length} itens · Área interna`}
          </p>
        </div>
          ...
          {KANBAN_COLS.map((col) => {
            ...
            return (
              <div key={col} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 22, fontWeight: 800, color: col === "Em Desenvolvimento" ? C.primary : C.text }}>
                  {count}
                </div>
            ...
            return (
              <div key={col}>
                ...
                >
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, background: C.card, borderRadius: 10, padding: "1px 7px" }}>
                    {colItems.length}
                  </span>
                </div>
        ...
        <Modal title="Nova Tarefa" onClose={() => setShowForm(false)}>
          <form onSubmit={handleCreate}>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: C.textSec, marginBottom: 8, letterSpacing: "0.04em" }}>
                TAGS
              </label>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        color: selected ? style.color : C.textMuted,
                        border: `1px solid ${selected ? style.border : C.border}`,
                        borderRadius: 4,
                        fontSize: 11,
                        fontWeight: 600,
                        padding: "3px 9px",
